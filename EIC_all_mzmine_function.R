...
EIC_all_mzmine <- function(df_drugs, file_list, output_folder){ 
  ...
  raw_data1 <- readMSData(files = samples, msLevel. = 1, mode = "onDisk", pdata = new("NAnnotatedDataFrame", pd))
  ...
  for (i in unique(df_selection$ion)){
    ...
    chr_raw <- chromatogram(raw_data1, rt = rtr, mz = mzr, aggregationFun = "max")
    plot(chr_raw)
